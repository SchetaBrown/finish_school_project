<script setup>
const props = defineProps(['directions', 'managers', 'types', 'olympiad']);
...
const { form, submit, updateValue } = useCustomForm({
    title: props.olympiad.title,
    description: props.olympiad.description,
    max_player_register_count: props.olympiad.max_player_register_count,
    register_start_date: props.olympiad.register_start_date,
    register_end_date: props.olympiad.register_end_date,
    start_date: props.olympiad.start_date,
    end_date: props.olympiad.end_date,
    olympiad_manager: props.olympiad.olympiad_manager_id,
    olympiad_direction: props.olympiad.olympiad_direction_id,
    types: props.olympiad.types,
});
        ...
        <form @submit.prevent="submit(route('admin.olympiad.store'))" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <component :is="input.component" :key="input.label" v-for="input in inputs" :label="input.label"
                    ...
                    @update-value="updateValue" :value="olympiad[input.name]" />
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <component :is="list.component" :key="list.label" v-for="list in lists" :label="list.label"
                    :name="list.name" :options="list.options" :form="form" @update-value="(data) => {
                        console.log(data)
                    }"
                    :value="olympiad[list.name]" />
            </div>
            <div class="flex flex-col gap-2">
                <Label label="Тип участия" />
                <div class="flex gap-2">
                    <InputBlock name="olympiad_types" type="checkbox" v-for="type in types.data" :key="type.id"
                        :checkboxOption="type.title" :placeholder="type.title" @update-value="updateValue"
                        :value="olympiad['types']" />
                </div>
            </div>
