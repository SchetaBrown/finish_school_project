...
const buttonCls = computed(() => {
  return (
    props?.btnClass ||
    "flex items-center justify-center gap-2 py-3 px-4 font-medium rounded-lg bg-indigo-600 text-sm text-white"
  );
});
...
=======
  ...
  <Teleport to="body">
    <div
      v-if="isOpen"
      class="fixed inset-0 z-50 flex items-center justify-center border border-gray-100"
    >
      <div class="absolute inset-0 backdrop-blur-sm" @click="close"></div>
      <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        <slot :close="close"></slot>
