    ...
    <div class="relative">
        ...
        <button @click="isVisibleList = !isVisibleList"
            ...
            <div class="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center mr-2">
                <span class="text-indigo-600 font-medium text-sm">{{ userData.name[0] }}{{
                    userData.surname[0] }}</span>
            </div>
        ...
        <div v-if="isVisibleList"
            ...
            <div class="px-4 py-3 border-b border-gray-100">
                <p class="text-sm font-medium text-gray-900">{{ userData.name }} {{ userData.surname }}</p>
                <p class="text-xs text-gray-400">{{ userData.email }}</p>
            </div>

            <div class="py-2">
                <Link :href="route(link.href)" v-for="link in links"
